...
- Run a skill's own test suite (`tests/run.sh` — fake `gh`, no network).
- `bash -n` syntax-check any shell script before running it live.
- `--dry-run` invocations of any skill script — `github-lockdown`'s `lockdown.sh`
...
- Any live invocation of `github-lockdown` against a real repo, which changes branch
  protection, rulesets, and repo settings — dry-run it first, show the human what it
  would do, then proceed.
- Running `neondb-branch`'s provisioning against a real Neon project. It has no
  `--dry-run`, and re-running it recreates the workspace branch and discards whatever
  data was in the old one — confirm the target project and branch with the human first.
...
runtime state). That makes parallel dispatch a good fit:
...
Present on `github-lockdown` (a plain-bash suite against `fake-gh` fixtures in the
skill's `tests/` dir) and `design-md` (a bash contract check plus a `node --test`
...
pins the Node version used by `design-md`'s `node --test` suite and by the
...
No `.env` files in this repo — it doesn't run a service, and nothing here reads an
environment variable of its own. The variables skills care about belong to the *target*
repo: `neondb-branch` reads `NEON_API_KEY`/`NEON_PROJECT_ID`/`NEON_PARENT_BRANCH` and
resolves workspace identity from Conductor, Orca, `WORKSPACE_NAME`, or the current git
branch. That precedence chain lives in its `SKILL.md`, not here.
...
- Frontmatter: `name` and `description` required, `metadata.author` +
  ...
  `weekly-risk-review` is at 1.2.0, `codebase-audit` at 0.2.0 after iterating). If a
...
- Scripts that manage state via hidden markers (e.g. namespaced HTML comments in
  issue bodies) should keep that marker-parsing narrowly anchored — a loose regex
  matching human-readable prose elsewhere in the same body is a real bug class in
  this repo's history, not a hypothetical.

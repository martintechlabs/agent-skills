...
describe('neondb-branch helpers', () => {
  ...
  describe('buildEnvFileContents (.env.neondb — mirrors DB vars AND Neon creds for archive)', () => {
    it('emits one single-quoted NAME=\'value\' line per entry, in the given order, after the header comment', () => {
      ...
      expect(lines[0]).toMatch(/^# Generated by scripts\/neondb-branch\.ts/)
      expect(lines[1]).toBe("DATABASE_URL='postgresql://user:pass@host/db'")
      expect(lines[2]).toBe("NEON_API_KEY='napi_abc123'")
      expect(lines[3]).toBe("NEON_PROJECT_ID='proj-123'")
      expect(lines[4]).toBe("NEON_PARENT_BRANCH='production'")
    })

    it('single-quotes values so dotenv-expand does not rewrite a literal $ inside them', () => {
      const contents = buildEnvFileContents({ DATABASE_URL: 'postgresql://user:p$ss@host/db?x=$1' })
      expect(contents).toContain("DATABASE_URL='postgresql://user:p$ss@host/db?x=$1'")
    })
  })
})
